...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
        ...
        auto poseIncrement = PoseIncrement<T>(arr);
        poseIncrement.apply(source, output);

        for(int g = 0; g < 3; g++) {
            residuals[g] = T(m_weight) * (output[g] - target[g]);
        }
...
public:
	...
	template <typename T>
	bool operator()(const T* const pose, T* residuals) const {
		// DONE: Implement the point-to-plane cost function.
		// The resulting 1D residual should be stored in the residuals array. To apply the pose 
		// increment (pose parameters) to the source point, you can use the PoseIncrement class.
		// Important: Ceres automatically squares the cost function.

        T source[3], output[3], target[3], normal[3];
        fillVector(m_sourcePoint, source);
        fillVector(m_targetPoint, target);
        ...
        T acc = T(0);
        for(int g = 0; g < 3; g++) {
            acc += T(m_weight) * normal[g] * (output[g] - target[g]);
        }
	...
	void pruneCorrespondences(const std::vector<Vector3f>& sourceNormals, const std::vector<Vector3f>& targetNormals, std::vector<Match>& matches) {
		...
		for (unsigned i = 0; i < nPoints; i++) {
			...
			if (match.idx >= 0) {
                ...
                double angle = acos(sourceNormal.dot(targetNormal) / sourceNormal.norm() / targetNormal.norm()) / M_PI * 180;
                if(angle > 60) {
                    match.idx = -1;
                }
			}
...
private:
	...
	Matrix4f estimatePosePointToPlane(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals) {
		const unsigned nPoints = sourcePoints.size();
		if (nPoints == 0) {
		    std::cout << "Distance between source and target points are to large" << std::endl;
		...
		for (unsigned i = 0; i < nPoints; i++) {
			...
			// DONE: Add the point-to-plane constraints to the system
			for (unsigned g = 0; g < 3; g++) {
                A(i * 4, g) = n[(g + 2) % 3] * s[(g + 1) % 3] - n[(g + 1) % 3] * s[(g + 2) % 3];
                A(i * 4, g + 3) = n[g];

                b(i * 4) += n[g] * d[g] - n[g] * s[g];
            }

            // DONE: Add the point-to-point constraints to the system
            // Constraint for x
            for (unsigned h = 0; h < 3; h++) {
                for (unsigned g = 0; g < 3; g++) {
                    A(i * 4 + 1 + h, g) = axes[h][(g + 2) % 3] * s[(g + 1) % 3] - axes[h][(g + 1) % 3] * s[(g + 2) % 3];
                    A(i * 4 + 1 + h, g + 3) = axes[h][g];

                    b(i * 4 + 1 + h) += axes[h][g] * d[g] - axes[h][g] * s[g];
                }
            }
