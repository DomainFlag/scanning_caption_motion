...
public:
    ...
    double Eval(const Eigen::Vector3d &_x) {
        return pow((_x.x() - m_center.x()), 2) + pow((_x.y() - m_center.y()), 2) + pow((_x.z() - m_center.z()), 2) -
               m_radius * m_radius;
    }
...
public:
    ...
    double Eval(const Eigen::Vector3d &_x) {
        double pow_xy = pow((_x.x() - m_center.x()), 2) + pow((_x.y() - m_center.y()), 2);
        double pow_radius_outer = pow(m_radius, 2);

        return pow((pow_xy + pow((_x.z() - m_center.z()), 2) + pow_radius_outer - pow(m_a, 2)), 2) -
               4 * pow_radius_outer * pow_xy;
    }
...
public:
    ...
    double Eval(const Eigen::Vector3d &_x) {
        ...
        double result = 0.0;
        for (unsigned int g = 0; g < m_numCenters; g++) {
            result += m_coefficents(g) * EvalBasis((m_funcSamp.m_pos[g] - _x).norm());
        }

        for (unsigned int g = 0; g < 3; g++) {
            result += m_coefficents(m_numCenters + g) * _x(g);
        }
...
#define phi(i, j) EvalBasis((m_funcSamp.m_pos[i]-m_funcSamp.m_pos[j]).norm());
    ...
    void BuildSystem() {
        ...
        for (unsigned int g = 0; g < m_numCenters * 2; g++) {
            for (unsigned int h = 0; h < m_numCenters; h++) {
                A(g, h) = phi(g, h);
            }

            for (unsigned int h = 0; h < 3; h++) {
                A(g, m_numCenters + h) = m_funcSamp.m_pos[g](h);
            }

            A(g, m_numCenters + 3) = 1;

            b(g) = m_funcSamp.m_val[g];
        }
