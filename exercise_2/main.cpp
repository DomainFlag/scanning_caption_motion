...
int main(int argc, char** argv) {
    ...
    std::string typeMeshInterpolation(argv[1]);
    std::string filenameOut = "../submission/post_" + typeMeshInterpolation + "_mesh.off";

    // implicit surface
    ImplicitSurface *surface;
    if (typeMeshInterpolation == "sphere") {
        surface = new Sphere(Eigen::Vector3d(0.5, 0.5, 0.5), 0.4);
    } else if (typeMeshInterpolation == "torus") {
        surface = new Torus(Eigen::Vector3d(0.5, 0.5, 0.5), 0.4, 0.1);
    } else if (typeMeshInterpolation == "hoppe") {
        surface = new Hoppe(filenameIn);
    } else {
        // Default RBF mesh smoothing
