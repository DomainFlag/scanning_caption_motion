...
public:
	...
	{
		// DONE: implement the implicit sphere formula using the member variables m_center and m_radius
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		// DONE: implement the implicit torus formula using the  variables m_center, m_radius (radius of the ring) and the radius m_a (small radius)
		return std::pow((_x - m_center).squaredNorm()+m_radius*m_radius-m_a*m_a, 2) - 4 * m_radius * m_radius * (_x - m_center).head<2>().squaredNorm();
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		Eigen::Vector3d p = m_pointcloud.GetPoints()[idx].cast<double> ();
		Eigen::Vector3d n = m_pointcloud.GetNormals()[idx].cast<double> ();

		// DONE: implement the evaluation using Hoppe's method (see lecture slides)
		return (_x - p).dot(n);
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		// DONE: eval the RBF function based on the coefficents stored in m_coefficents
		...
		result += m_coefficents.segment(m_numCenters, m_numCenters+2).dot(_x);
...
#define phi(i,j) EvalBasis((m_funcSamp.m_pos[i]-m_funcSamp.m_pos[j]).norm())
	...
	void BuildSystem()
	{
		...
		// DONE fill the matrix A and the vector b as described in the exercise sheet
